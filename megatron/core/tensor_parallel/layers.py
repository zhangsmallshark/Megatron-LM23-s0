...
class RowParallelLinearNoComm(torch.nn.Module):
    """Linear layer with row parallelism without communication.
...
class ColRowParallelLinear(torch.nn.Module):
    """Linear layer with column & Row parallelism.
    ...
    def forward(self, inputs_: List[torch.Tensor], weight: Optional[torch.Tensor] = None):
        """Forward of ColumnRowParallelLinear

        Args:
            inputs_: List 3D tensor whose order of dimension is [sequence, batch, hidden]
        ...
        input0_, input1_ = inputs_
        if (
            self.async_tensor_model_parallel_allreduce
            or self.sequence_parallel
            or self.explicit_expert_comm
        ):
            input_parallel0 = input0_
            input_parallel1 = input1_
        else:
            input_parallel0 = copy_to_tensor_model_parallel_region(input0_)
            input_parallel1 = copy_to_tensor_model_parallel_region(input1_)

        input0, input1 = input_parallel0, input_parallel1
